# A Node.js BigBang Client!

![BigBang](https://github.com/lefter1s)

node-BigBang is a BigBang client for Node.js. It is a fork of the excellent Kapitalize Bitcoin Client (now removed from GitHub) intended for use with BigBang. The purpose of this repository is:

* Provide a one-stop resource for the Node.js developer to get started with BigBang integration.
* Prevent would-be BigBang web developers worrying whether a Bitcoin client will work out of the box.
* Promote Node.js development of BigBang web apps.
* Identify and address any incompatibilities with the BigBang and Bitcoin APIs that exist now and/or in the future.

## Dependencies

You'll need a running instance of [BigBangd](https://github.com/Lefter1s/BigBang) to connect with. 

Then, install the node-BigBang NPM package.

`npm install git://github.com/Lefter1s/node-BigBang/`

## Examples

Some code examples follow below, but for more complete examples, see [these snippets](https://github.com/BigBang-project/BigBang-code-snippets), or this [wallet app](comingsoon...) which was created to to test this module.

```js
var BigBang = require('node-BigBang')()

BigBang.auth('myusername', 'mypassword')

BigBang.getDifficulty(function() {
...
```js
BigBang.getNewAddress(function(err, address) {
...
```js
BigBang.exec('getNewAddress')

BigBang.exec('getbalance', function(err, balance) {
...
<td> <b>Currently only available on testnet</b> Add a nrequired-to-sign multisignature address to the wallet. Each key is a BigBang address or hex-encoded public key. If [account] is specified, assign address to [account]. </td>
<td> N
</td></tr>
<tr>
<td> backupwallet </td>
...
<tr>
<td> dumpprivkey </td>
<td> [BigBangaddress] </td>
<td> Reveals the private key corresponding to <BigBangaddress< </td>
<td> Y
</td></tr>
<tr>
<td> encryptwallet </td>
...
<td> [account] </td>
<td> Returns the current BigBang address for receiving payments to this account. </td>
<td> N
</td></tr>
<tr>
...
<td> getgenerate </td>
<td> </td>
<td> Returns true or false whether BigBangd is currently generating hashes </td>
<td> N
</td></tr>
<tr>
...
<td> getnewaddress </td>
<td> [account] </td>
<td> Returns a new BigBang address for receiving payments.  If [account] is specified (recommended), it is added to the address book so payments received with the address will be credited to [account]. </td>
<td> N
</td></tr>
<tr>
...
<td> [BigBangaddress] [minconf=1] </td>
<td> Returns the total amount received by <BigBangaddress< in transactions with at least [minconf] confirmations. While some might consider this obvious, value reported by this only considers *receiving* transactions. It does not check payments that have been made *from* this address. In other words, this is not "getaddressbalance". Works only for addresses in the local wallet, external addresses will always show 0. </td>
<td> N
</td></tr>
<tr>
...
<p>To get a list of accounts on the system, execute BigBangd listreceivedbyaddress 0 true
</p>
</td>
<td> N
</td></tr>
...
<tr>
<td> sendfrom </td>
<td> [fromaccount] [toBigBangaddress] [amount] [minconf=1] [comment] [comment-to] </td>
...
<td> stop </td>
<td> </td>
<td> Stop BigBang server. </td>
<td> N
</td></tr>
<tr>
<td> validateaddress </td>
<td> [BigBangaddress] </td>
<td> Return information about [BigBangaddress]. </td>
<td> N
</td></tr>
<tr>
<td> verifymessage </td>
<td> [BigBangaddress] [signature] [message] </td>
...
BigBang.set('pass', 'somn')
BigBang.set({port:7142})
...
You may pass an optional function `passphrasecallback` to the `node-BigBang` initialization function to manage wallet unlocks. `passphrasecallback` should be a function accepting three arguments:
...
By default `BigBangd` exposes its JSON-RPC interface via HTTP; that is, all RPC commands are transmitted in plain text across the network! To secure the JSON-RPC channel you can supply `BigBangd` with a self-signed SSL certificate and an associated private key to enable HTTPS. For example, in your `BigBang.conf`:

    rpcssl=1
    rpcsslcertificatechainfile=/etc/ssl/certs/BigBangd.crt
    rpcsslprivatekeyfile=/etc/ssl/private/BigBangd.pem

In order to securely access an SSL encrypted JSON-RPC interface you need a copy of the self-signed certificate from the server: in this case `BigBangd.crt`. Pass your self-signed certificate in the `ca` option and set `https: true` and node-BigBang is secured!
    
```js
var fs = require('fs')

var ca = fs.readFileSync('BigBangd.crt')
...
nodeunit test/test-node-BigBang.js
```

## Bounties

[BigBang](http://www.BigBangcoin.info) donation address is B7EzQNeCsuCTeZbC2Go43ykEgN3DcD6ify
